...
void special(object attacker, object target)
{
  ...
  if (!check)
  {
    ...
    {
      attacker->targetted_action("$N $vattempt a backward trip on $t, but only $vmanage a punch to the face.", target, limb);
      target->hurt_us(1 + random(5), "head");
    }
    else
      attacker->targetted_action("$N $vattempt to perform a backward trip, but $t $v1prevent $n from getting into the right position.", target);
    return;
  }
  ...
  case 0:
    ...
    if (limb)
    {
      if (random(2))
      {
        attacker->targetted_action("$N $vattempt a backward trip on $t, but only $vmanage a kick to $p1 $o.", target, limb);
