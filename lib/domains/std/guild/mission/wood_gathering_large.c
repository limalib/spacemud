...
// MISSION: Wood gathering (Large)
int wood_total = 0;
int minimum_wood = 100;
string hiscore_who;
int hiscore;
...
{
   set_length(60);
   set_mission_data(2, "Wood gathering (Large)");
   set_receive_items(1);
   set_favour(1500);
   set_description("Help <guild> gather wood for heating up their offices. You need to find "
                   "and hand in items within 15 minutes that can be salvaged to " +
                   minimum_wood +
                   " wood scraps. "
                   "\n\nNOTICE: Adding wood scraps to our storage will *not* "
                   "help for this mission.");

   set_materials("steel scrap:40,wood scrap:25");
}
...
int run_mission()
{
   ...
   if (complete())
      channel_msg(query_name() + " mission is COMPLETED with " + query_length() + " of " + query_original_length() +
                  " minutes left. " + "You can still contribute to earn favour. Our total is now at " + wood_total +
                  ". " + (hi_str ? hi_str : ""));
   else
   {
      channel_msg(query_name() + " mission is ongoing with " + query_length() + " of " + query_original_length() +
                  " minutes left. " + "Find items that can be salvaged to wood scraps and give them to me. ");
      channel_msg(wood_total + " wood scraps received of minimum " + minimum_wood + ". " + (hi_str ? hi_str : ""));
   }
   return ::run_mission();
}
...
void receive_item(object ob)
{
   ...
   if (ob->is_salvageable())
   {
      ...
      wood = filter_array(salvage, ( : $1->id("wood scrap") :));
      salvage -= wood;
...
{
   int f = query_favour();
   float favour_pr_wood = 1.0 * f / (wood_total || 1);
   ...
   foreach (string player, int num in wood_received)
   {
      ...
      if (body)
      {
         int fave = to_int(num * favour_pr_wood);
         if (complete())
         {
