...
- Changed debug class for errors generated by wizards to be ':<wiz name>'.  This is so wizzes can selectively enable
which wizards they want to receive errors for.
...
#define OUTPUT                                                                                                         \
   (["file":0,                                                                                                         \
              "line":1, "program":2, "trace":3, "error":4, "arguments":5, "object":6, "interactive":7, "context":8,    \
          "function":9, "locals":10, ])
...
private : string
          log_current_error(mapping err_map, int caught);
...
private
void build_error_data(mapping error_map, int caught)
{
   ...
   error_data.uid =
       error_data.interactive ? capitalize((string)error_data.interactive->query_userid() || "(none)") : "(none)";
...
void build_output_data()
{
   ...
   output_data.trace = sprintf("\n\n\n%sTrace:%s\n\n%s", HIW, NOR,
                               implode(map_array(error_data.trace,
                                                 //(: sprintf("Line: %O File: %O Function: %O Object: %O Program: %O",
                                                 (
                                                     : sprintf("Line %-4.4O %O %s() Ob: %O", (int)$1["line"],
                                                               (string)$1["file"], (string)$1["function"],
   ...
   output_data.context = sprintf("\n\n\n%sContext:%s\n\n%s%s%s", HIW, NOR, (*f)(-3, 3), (*f)(0, 1), (*f)(1, 3));
}

protected
void error_handler(mapping error_map, int caught)
{
   string t;
   ...
   if (objectp(error_data.interactive))
   {
      tell(error_data.interactive,
           sprintf("%s%s\n%s\n", sprintf("\n%s\n%s\n\n%s", BORDER, ctime(), output_data.error), output, BORDER));
   }
   send_to_channel();
}
...
void send_to_channel()
{
   CHANNEL_D->deliver_string(
       "errors", sprintf("%s (%s) Error logged to %s\n" + "%s %s\n" + "%s %s\n", CHANNEL_TAG, error_data.uid,
                         error_data.logfile, CHANNEL_TAG, error_data.error, CHANNEL_TAG,
                         master()->trace_line(error_data.ob, error_data.program, error_data.file, error_data.line)));
}
