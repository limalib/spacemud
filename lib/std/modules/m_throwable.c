...
{
   this_body()->start_fight(target);
   this_body()->add_event(target, this_object(), choice(target->query_vital_limbs()), evaluate(damage_func));
}
...
mixed land_throwable(object target, int success)
{
   if (success && target)
   {
      ...
      this_body()->start_fight(target);
      this_body()->add_event(target, this_object(), target->query_random_armour_slot(), damage);
